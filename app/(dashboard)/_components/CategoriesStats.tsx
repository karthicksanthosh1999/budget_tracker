...
import { GetBalanceStatusResponseType } from "@/app/api/stacks/categories/route";
...
const CategoriesStats = ({ from, to, userSettings }: Props) => {
  const statsQuery = useQuery<GetBalanceStatusResponseType>({
    ...
    queryFn: () =>
      fetch(
        `/api/stacks/categories?from=${DateToUTCDate(from)}&to=${DateToUTCDate(
          to
        )}`
      ).then((res) => res.json()),
  });
...
function CategoriesCard({
  ...
  data: GetBalanceStatusResponseType;
}) {
  const filteredDate = data.filter((el) => el.type === type);
  ...
  return (
    <Card className="h-80 w-full">
      <CardHeader>
        <CardTitle className="grid grid-flow-row justify-between gap-20">
