...
const columns: ColumnDef<TransactionHistoryRow>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="capitalize">
          <div className="text-muted-foreground">{formatterDate}</div>
        </div>
      );
    },
  },
...
const TransactionTable = ({ from, to }: { from: Date; to: Date }) => {
  ...
  const categoriesOptions = useMemo(() => {
    ...
    history.data?.forEach((transactions) => {
      categoriesMap.set(transactions.category, {
        value: transactions.category,
        label: `${transactions.categoryIcon} ${transactions.category}`,
      });
    });
    ...
    <>
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant={"ghost"} className="h-2 2-8 p-0">
